...
def 计算每批分类平均精确度(有物体判断为有物体, 置信度列表, 预测的标签列表, 目标分类列表):
    ...
    平均精确度, 精确度, 召回率 = np.array(平均精确度, dtype="int32"), np.array(精确度, dtype="int32"), np.array(召回率, dtype="int32")
...
def 构建目标列表(预测的盒子列表, 预测的分类列表, 目标列表, 锚定盒列表, 忽略用阈值):
    ...
    分类的掩码列表[图片索引列表, 最佳_交并比索引列表, 目标盒子x取整列表, 目标盒子y取整列表] = (预测的分类列表[图片索引列表, 最佳_交并比索引列表, 目标盒子x取整列表, 目标盒子y取整列表].argmax(-1) == 目标标签列表).float()
    交并比分数列表[图片索引列表, 最佳_交并比索引列表, 目标盒子x取整列表, 目标盒子y取整列表] = 盒子边界的交并比(预测的盒子列表[图片索引列表, 最佳_交并比索引列表, 目标盒子x取整列表, 目标盒子y取整列表], 目标盒子列表, 不转换坐标=False)
