...
def 创建模块(模块定义列表):
    ...
    for 模块_索引, 模块_定义 in enumerate(模块定义列表):
        ...
        if 模块_定义["类型"] == "卷积":
            ...
            if 是否批归一化:
                模块列表.add_module(f"批归一化_{模块_索引}", nn.BatchNorm2d(过滤器数量, momentum=0.9, eps=1e-5))
            if 模块_定义["激活函数"] == "泄露型线性整流函数":
                模块列表.add_module(f"泄露型_{模块_索引}", nn.LeakyReLU(0.1))
        elif 模块_定义["类型"] == "最大池化":
            内核大小 = int(模块_定义["大小"])
            步长 = int(模块_定义["步长"])
            if 内核大小 == 2 and 步长 == 1:
                模块列表.add_module(f"_调试_填充_{模块_索引}", nn.ZeroPad2d((0, 1, 0, 1)))
            最大池化 = nn.MaxPool2d(kernel_size=内核大小, stride=步长, padding=int((内核大小 - 1) // 2))
            模块列表.add_module(f"最大池化_{模块_索引}", 最大池化)
        elif 模块_定义["类型"] == "上采样":
            某个上采样 = 上采样(比例因子=int(模块_定义["步长"]), 模式="nearest")
            模块列表.add_module(f"上采样{模块_索引}", 某个上采样)
        elif 模块_定义["类型"] == "路径":
            层数列表 = [int(x) for x in 模块_定义["层数"].split(",")]
            过滤器数量 = sum([输出_过滤器_列表[1:][i] for i in 层数列表])
            模块列表.add_module(f"路径_{模块_索引}", 空层())
        elif 模块_定义["类型"] == "捷径":
            过滤器数量 = 输出_过滤器_列表[1:][int(模块_定义["来自"])]
            模块列表.add_module(f"捷径_{模块_索引}", 空层())
        elif 模块_定义["类型"] == "我只看一次":
