...
class 我只看一次层(nn.Module):
    ...
    def forward(self, 输入列表, 目标列表=None, 图片维度=None):
        ...
        # 转成布尔型，8位无符号整形在索引方式中已弃用
        有物体的掩码列表 = 有物体的掩码列表.bool()
        无物体的掩码列表 = 无物体的掩码列表.bool()

        x的损失值 = self.均方误差损失值函数(x[有物体的掩码列表], 目标中x的列表[有物体的掩码列表])
        y的损失值 = self.均方误差损失值函数(y[有物体的掩码列表], 目标中y的列表[有物体的掩码列表])
        宽的损失值 = self.均方误差损失值函数(宽[有物体的掩码列表], 目标中宽的列表[有物体的掩码列表])
        高的损失值 = self.均方误差损失值函数(高[有物体的掩码列表], 目标中高的列表[有物体的掩码列表])
...
class 黑夜网络(nn.Module):
    def __init__(self, 配置文件路径, 图片尺寸=416):
        ...
        self.我只看一次层列表 = [层[0] for 层 in self.模块列表 if hasattr(层[0], "指标字典")]
        self.图片尺寸 = 图片尺寸
        self.见过 = 0
    ...
    def forward(self, 输入列表, 目标列表=None):
        ...
        return 我只看一次层的输出列表 if 目标列表 is None else (损失值, 我只看一次层的输出列表)
